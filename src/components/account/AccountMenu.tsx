...
import { avatarChangedEvent } from "@/services/account-avatar-store"
...
export function AccountMenu({ profile, variant = "header", collapsed = false, className }: AccountMenuProps) {
  ...
  const trigger = isSidebar ? (
    <button ref={triggerRef} type="button" onPointerDown={markPointerInteraction} onKeyDown={markKeyboardInteraction} className={cn("group flex min-w-0 items-center gap-2.5 rounded-md px-1 py-2 text-left transition-colors hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500", collapsed && "justify-center px-0", className)} aria-label={`Hesap menüsü: ${profile.fullName}, ${profile.roleLabel}`} title={collapsed ? `${profile.fullName} · ${profile.roleLabel}` : undefined}>
      <UserAvatar {...profile} avatar={avatar} className="size-9 shrink-0" />
      <div aria-hidden={collapsed} className={cn("min-w-0 flex-1 leading-tight transition-[opacity,transform] duration-150 ease-out motion-reduce:transition-none", collapsed ? "w-0 translate-x-1 opacity-0" : "translate-x-0 opacity-100 delay-100")}>
      ...
      {!collapsed && <ChevronDown aria-hidden="true" className="size-4 shrink-0 text-slate-400 transition-transform duration-150 ease-out group-data-[state=open]:rotate-180 motion-reduce:transition-none" />}
    </button>
  ) : (
    <button ref={triggerRef} type="button" onPointerDown={markPointerInteraction} onKeyDown={markKeyboardInteraction} className={cn("group flex min-w-0 items-center gap-2 rounded-md px-1 py-1 transition-colors hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500", className)} aria-label={`Hesap menüsü: ${profile.fullName}, ${profile.roleLabel}`}>
      ...
      <ChevronDown aria-hidden="true" className="size-3.5 shrink-0 text-slate-400 transition-transform duration-150 ease-out group-data-[state=open]:rotate-180 motion-reduce:transition-none" />
    </button>
  )
