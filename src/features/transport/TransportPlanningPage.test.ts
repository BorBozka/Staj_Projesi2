...
describe("TransportPlanningPage table polish", () => {
  it("uses the shared divider treatment and keeps totals in the footer", () => {
    ...
    expect(componentSource).toContain('className="px-3 py-2">Tarih / saat</th>')
    expect(componentSource).toContain('className="whitespace-nowrap px-3 py-1 font-medium"')
    expect(componentSource).toContain('className="leading-4 font-medium text-slate-900')
