...
export function TransportPlanningPage() {
  ...
  const { ref: recordsCardViewportRef, height: recordsCardHeight } = useFillViewportHeight(14, [allVisibleAssignments.length])
  ...
  return <>
    ...
    <div className="min-w-0 -mb-3 space-y-2 md:space-y-2.5">
    ...
    <div ref={recordsCardViewportRef} className={cn("scroll-mt-3 flex h-full min-h-0 flex-col overflow-hidden rounded-lg border bg-card shadow-panel", recordsCardHeight === undefined && "min-h-[20rem]")} style={recordsCardHeight !== undefined ? { height: recordsCardHeight } : undefined}>
      <div className="min-h-0 flex-1 overflow-x-auto overflow-y-hidden">
        {visibleAssignments.length === 0 ? <p className="flex h-full items-center justify-center px-3 text-center text-sm text-slate-500">{date && companyId && facilityId ? "Seçili tarih ve tesis için planlı atama yok." : "Yaklaşan aktif atama yok."}</p> : <table className="h-full w-full min-w-[760px] text-left text-xs"><thead className="border-b bg-slate-50 text-[11px] font-medium text-slate-500"><tr><th className="px-3 py-2">Tarih / saat</th><th className="px-3 py-2">Araç</th><th className="px-3 py-2">Şoför</th><th className="px-3 py-2">Görev / amaç</th><th className="px-3 py-2">DURUM</th></tr></thead><tbody className="divide-y">{visibleAssignments.map((assignment) => <tr key={assignment.id} role="button" tabIndex={0} aria-label={`${assignment.vehicleName} ${assignment.vehicleLicensePlate} araç görevi detaylarını aç`} className={cn("record-row-hover group cursor-pointer text-slate-700 transition-colors hover:bg-blue-50/60 focus-visible:bg-blue-50/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-600", assignment.status === "CANCELLED" && "bg-slate-50 text-slate-400 hover:bg-slate-100 focus-visible:bg-slate-100")} onClick={() => setViewingAssignment(assignment)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setViewingAssignment(assignment) } }}><td className="whitespace-nowrap px-3 py-1 font-medium">{formatTransportAssignmentSchedule(assignment)}</td><td className="px-3 py-1"><p className="leading-4 font-medium text-slate-900 transition-colors group-hover:text-blue-800 group-focus-visible:text-blue-800">{assignment.vehicleName}</p><p className="mt-0.5 leading-4 text-slate-500">{assignment.vehicleLicensePlate}</p></td><td className="px-3 py-1 font-medium text-slate-900">{assignment.driverName}</td><td className="px-3 py-1">{assignment.purpose}</td><td className="px-3 py-1"><span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-semibold text-blue-700">Planlı</span></td></tr>)}{Array.from({ length: Math.max(0, TRANSPORT_PAGE_SIZE - visibleAssignments.length) }).map((_, index) => <tr key={`filler-${index}`} aria-hidden="true" className={cn("pointer-events-none select-none", index > 0 && "border-transparent")}><td className="px-3 py-1"><p className="font-medium text-transparent">&nbsp;</p><p className="mt-0.5 text-[11px] text-transparent">&nbsp;</p></td><td className="px-3 py-1 text-transparent">&nbsp;</td><td className="px-3 py-1 text-transparent">&nbsp;</td><td className="px-3 py-1 text-transparent">&nbsp;</td><td className="px-3 py-1 text-transparent">&nbsp;</td></tr>)}</tbody></table>}
      </div>
    <PaginationFooter
      page={page}
      pageCount={pageCount}
